...
describe JobDatabaseManager::BaseJob do

  def build_klass
    klass = Class.new do
      include JobDatabaseManager::BaseJob
    end
    klass.new
  end
  ...
  describe '#default_job_db_user' do
    it 'should return the db user for the job' do
      expect(klass).to receive(:job_db_name).and_return('foo_db')
  ...
  describe '#db_connection' do
    context 'when adapter exists' do
      it 'should invoke it with params' do
        adapter = 'FooAdapter'
        expect(klass).to receive(:launcher).and_return(double('launcher'))
